...
var ButtonGroup = React.createClass({
  ...
  //           item.className = item.className.replace(/(\s|^)current-list(\s|$)/,' ');
		...
		return (
                <input type="button" value={this.props.value} onClick={this.handleClick} onTouchStart={this.handleTapEvent} onTouchEnd={this.handleEndEvent}/>
			)
	}
});
        ...
        if(index == 0){
        	return (
                <div className={this.props.name}>
                    {
                    	this.props.items.map(function(child,i){
							return <input type="button" ref={["btn",i].join("")} className={i == index ? _this.newClass : ""} value={child} onClick={_this.handleClick.bind(_this,i)} />;
						})
                    }
                    <ButtonGroup value="button-group"></ButtonGroup>
                    <input type="text" value="ceshi"/>
                </div>
			)
        }
		return (
